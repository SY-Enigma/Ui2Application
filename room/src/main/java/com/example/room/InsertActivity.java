...
import com.example.room.databinding.ActivityInsertBinding;

public class InsertActivity extends AppCompatActivity implements View.OnClickListener {
    private Student currentStudent;
    private boolean isUpdate = false; // 添加或更新的标识符
    private ActivityInsertBinding binding;
    private InfoRoomDatabase infoDB = InfoRoomDatabase.getInstance(InsertActivity.this);
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.btn_confirm) {
            // 将输入的数据封装成student对象
            Student student = new Student(binding.etName.getText().toString(),
            ...
            if(isUpdate) {
                ...
                infoDB.getStudentDao().update(student);
            } else {
                // 插入数据
                infoDB.getStudentDao().insert(student);
            }
